...
[We previously](#normal) explored samples drawn from a normal distribution. A key takeaway was the Z-transformation:  
...
* First, we cover the standard calculations for a t-distribution.
* Next, we show how to use the t-distribution to find confidence intervals for an estimate of the mean.
* Then, we introduce the one-sample and paired t-test as applications for null hypothesis significance testing.

  * Along the way, we note the assumptions behind these tests.
  * We also point out alternatives you might use when those assumptions aren’t met.
* We take a short detour to revisit Cohen’s as a measure of effect size, and discuss the difference between statistical and practical significance.   

As always, we end with a chapter summary!
