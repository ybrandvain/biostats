...
<article class="drop-cap">The $F$ distribution is often introduced after the t-distribution to clear a path to comparing means of more that two groups. This makes sense, as the $F$ lends itself to this application. But, in my view at least, that introduction to the $F$ does it a disservice. More than simply offering a way to compare more than two groups, the $F$ provides us a new way to think about variability in data. Specifically, the $F$ allows us to "partition variance" into variance into  components associated with variables in our model and "residual" variance that is not in our model. </article><br>
Here, I introduce the $F$ as a tool to understand variability within and between two samples. By focusing our first application of the $F$ to the topic we covered previously (just in the last chapter, in fact), we can use our additional brain power to think about what the $F$ is going on!.
...
In the previous section, we saw that at sawmill road, pink parviflora RILs attracted more pollinators than did white parviflora RILs. We cared about this because we want to know if petal color influences hybridization -- a question we will aress later. 

But there is a bigger motivating question than what happens in an experiment - we want to know what has actually happened (and what is happening) in nature. In this chapter we get at the question of "does petal color influence hybridization?" in a somewhat backwards way. Instead of asking if pink flowers set more hybrid seed than white flowers, we analyze sequence data from pink and white flowered parviflora found in natural hybrid zones and ask if one petal color is associated with more introgression of xantiana ancestry. 
...
- We then show how to calculate the sum of squares error an sums of squares model and related measures.   
