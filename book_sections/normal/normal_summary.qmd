...
**Q8.** Do you think baby weights are actually perfectly normal? If not, how might they deviate?

```{=html}
<div style="width:100%; height:300px; overflow:hidden; border:none;">
  <iframe 
    src="https://docs.google.com/forms/d/e/1FAIpQLSf4HtvBsUEww-EQ4MgHxENxjjyq4W_gi3JWKitlSYHQKYG1ZA/viewform?embedded=true" 
    style="margin-top:-180px; height:330px; width:100%; border:none;" 
    frameborder="0">
  </iframe>
</div>
```
...
* **Standard Error (SE)**: The standard deviation of the sampling distribution. For the sample mean, of a normally distributed variable, it is calculated as $\sigma / \sqrt{n}$.
...
* **Z-test**: A test of the null hypothesis that a population's true mean equals its proposed null value, $\mu_0$. To do we find the distance (in standard errors) between our estimate, $\bar{x}$, and its null value $\mu_0$, as $Z=\frac{\bar{x}-\mu_0}{SE_x}$, where $SE_x$ is the standard error. We then find the two-tailed p-value as `2 * pnorm(q=abs(Z), sd =1, lower.tail = TRUE)`.
