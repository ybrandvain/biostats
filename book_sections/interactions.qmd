...
tibble( yumm = c(0,5,3,-5), drink = c("Water","OJ","Water","OJ"), dental_hygine = c("floss", "floss","toothpaste","toothpaste")) %>% mutate(drink = fct_rev(drink)) %>%
  ...
  geom_text(data = tibble( yumm = c(-1,4), drink = c("Water","Water"), dental_hygine = c("floss","toothpaste")) %>% mutate(drink = fct_rev(drink)),
  aes(label = dental_hygine), show.legend = FALSE,size = 10)+
