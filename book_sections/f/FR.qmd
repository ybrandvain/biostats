...
In the previous sections we worked through the math and the theory, and even how we could use R to do our calculations or us. This is all useful and I hope it helps you understand what is going on behind the scenes. But now that we know what's going on, we can have R do our work for us. Here, I show you two ways to have R calculate F and conduct an anova test for us. 
...
`broom`'s `tidy()` function provides this out put in a "tidy" format, its `glance()`  function shows some relevant summaries of the model (including $R^2$).
...
Alternatively, we can fit a linear model and then hand it to [`anova()`](https://stat.ethz.ch/R-manual/R-devel/RHOME/library/stats/html/anova.html)
 to create the ANOVA table. This produces the same output as `aov() |> summary()`, but the intermediate object is an lm object rather than an aov object. Sometimes one format is just easier to work with than the other, but your answers will be the same.
