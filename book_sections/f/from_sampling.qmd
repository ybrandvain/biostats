...
library(tidyr)
source("../../_common.R") 
clarkis_hz <-read_csv("~/Downloads/clarkiaContactZone_bio3272_petal_color_30_70_refined.csv")|>
  clean_names()|>
...
Remember our "statistical" view of were data comes from. We imagine that the data we observe are a sample of size $n$ from a population with a true mean $\mu$ and standard deviation $\sigma_x$. Although we can't know these true parameters, we estimate them using the sample mean $\bar{x}$ and standard deviation $s$
...
#| echo: false
#| label: fig-anova
#| fig-cap: "Tha F distirbution is magic."
...
We can use these values to calculate the ratio of variance among and within groups, $F$. **Notice** that when our two samples come from the same population, we expect $F$ to euqal one (save some sampling error).
